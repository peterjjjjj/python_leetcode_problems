...
def max_depth_review_bfs(root: TreeNode) -> int:
    if not root:
        return 0
    queue = deque()
    queue.append(root)
...
def max_depth_recursive(root: TreeNode, depth: int = 0) -> int:
    if root is None:
        return depth

    left_depth = max_depth_recursive(root.left, depth+1)
    right_depth = max_depth_recursive(root.right, depth+1)
...
if __name__ == "__main__":
    ...
    print(max_depth_review_bfs(test))
    print(max_depth_recursive(test))
